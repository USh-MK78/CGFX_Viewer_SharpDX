...
namespace CGFX_Viewer_SharpDX.MeshBuilderComponent.BoundManager
{
    public sealed class CustomGeometryBoundManager : IDisposable
    {
        #region Properties
        ...
        private Node.CGFXGeometryNode elementCore;

        public CustomGeometryBoundManager(Node.CGFXGeometryNode core)
        {
        ...
        {
            var sceneNodeProperty = elementCore.GetType().GetRuntimeFields();
            var TotalModelMatrixInternal_Field = sceneNodeProperty.ToList().Find(x => x.Name == "TotalModelMatrixInternal");
            var TotalModelMatrixInternal = TotalModelMatrixInternal_Field.GetValue(elementCore);
            ...
            else
            {
                ...
                if (!HasInstances)
                {
                    ...
                    BoundsWithTransform = Bounds.Transform((Matrix)TotalModelMatrixInternal);
                    RaiseOnTransformBoundChanged(BoundsWithTransform, oldBound);
                }
                else
                {
        ...
        private void UpdateBoundingSphere()
        {
            ...
            else
            {
                ...
                if (!HasInstances)
                {
                    ...
                    BoundsSphereWithTransform = BoundsSphere.TransformBoundingSphere((Matrix)TotalModelMatrixInternal);
                    RaiseOnTransformBoundSphereChanged(BoundsSphereWithTransform, oldSphere);
                }
                else
                {
