...
using HelixToolkit.Wpf.SharpDX.Model.Components;
...
{
    public abstract class CGFXGeometryNode : SceneNode, IHitable, IThrowingShadow, IInstancing, IBoundable, IApplyPostEffect
    {
        #region Properties
        ...
        /// The bound manager.
        /// </value>
        public readonly BoundManager.CustomGeometryBoundManager BoundManager;
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CGFXGeometryNode"/> class.
        /// </summary>
        public CGFXGeometryNode()
        {
            BoundManager = new BoundManager.CustomGeometryBoundManager(this);
        ...
        {
            var sceneNodeProperty = this.GetType().GetRuntimeFields();
            var TotalModelMatrixInternal_Field = sceneNodeProperty.ToList().Find(x => x.Name == "TotalModelMatrixInternal");
            var TotalModelMatrixInternal = TotalModelMatrixInternal_Field.GetValue(this);
            ...
            if (CanHitTest(context) && PreHitTestOnBounds(context))
            {
                if (this.InstanceBuffer.HasElements)
                {
                    ...
                    {
                        if (OnHitTest(context, modelMatrix * (Matrix)TotalModelMatrixInternal, ref hits))
                        {
                            hit = true;
                ...
                {
                    return OnHitTest(context, (Matrix)TotalModelMatrixInternal, ref hits);
                }
            }
            else
            {
